...
def getIdFromCode(codeFunction):
    switcher = {
        'CAB': 0,
        'BSA': 1,
        'CAT': 2,
    ...
    return switcher.get(codeFunction,"Invalido")
...
for b in range(len(states)):
    ...
    for a in range(len(cities)):
        if (states[b].get('code') == cities[a].get('code')):
            parsed.append('{"id":'+str(a)+',"name":"'+cities[a].get('name')+'","state_id":'+str(b)+'},')
    parsed.append('},')


for b in range(len(states)):
    parsedStates.append('{"id":'+str(b)+',"name":"'+states[b].get('name')+'"},')

fStates = open("parsedJsonStateArg.txt","w")
for b in range(len(states)):
    fStates.write(parsedStates[b])
fStates.close()
